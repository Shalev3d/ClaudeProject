...
def train_model(config):
    ...
    if fpga_accelerator:
        ...
        # Show real FPGA cycle data if available
        if stats.get('has_real_cycle_data', False):
            print(f"    Real FPGA Board Cycles:")
            print(f"      Total FPGA cycles: {stats['real_fpga_cycles_total']:,}")
            print(f"      Average cycles/operation: {stats['real_fpga_cycles_average']:,.0f}")
            print(f"      Measured from actual K5 hardware!")
        else:
            print(f"    Simulated mode - compile C program for real cycle measurements")
            
        print("K5 FPGA accelerator session completed")

    # Signal training completion to C program FPGA server
    if config.get('use_fpga', False):
        try:
            with open("python_training_complete.txt", "w") as f:
                f.write("TRAINING_COMPLETE\n")
            print(" Signaled training completion to FPGA server")
        except:
            pass
